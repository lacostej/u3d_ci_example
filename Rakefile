...
task :u3d_install do
  ...
  system("u3d install --verbose") or exit(-1)
  system("u3d dependencies") or exit(-1)
end

task :u3d_load_save_scenes do
  system("u3d -r -- -logFile './editor.log' -executeMethod U3d.EditorRun.LoadSaveScenes -quit -batchmode") or exit(-1)
end
